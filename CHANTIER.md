...
### 2026-07-25 : refonte de l'écran d'accueil (page pseudo/join) inspirée d'un `design.md` hors-sujet

**Contexte :** un fichier `design.md` à la racine décrivait le design handoff d'une **autre appli** ("NoLimits", dashboard de jeux de cartes casino — La Chocholle, PMU, Le 99 — en Tailwind v4, route `/cards`), sans rapport de contenu avec Petit Bac. Décision validée avec l'utilisateur : reprendre uniquement l'esprit visuel (fond dégradé plein écran, motifs flottants, hiérarchie typo bold), pas le contenu, et l'appliquer au véritable écran d'accueil (`App.vue`, branche `phase === 'PSEUDONYM'`), pas au reste de l'app.

**Nouveau variant `SummerDecor` `hero-vivid`** (`src/components/SummerDecor.vue`) : calque `position: fixed; inset: 0` avec dégradé été saturé + motifs été en couleur (palmier/cocktail/pastèque/soleil, mêmes illustrations que les variants `hero`/`scatter` existants) + 2 halos flous (`::before`/`::after`, technique `.ambient-blobs`) + grain léger + petites bulles. Monté/démonté avec la phase (`v-if` sur `phase !== 'PSEUDONYM'` dans `App.vue`), donc jamais visible sur les autres écrans.

**Pièges rencontrés (dans l'ordre) :**
1. **`position: fixed` + fond plein écran sans `z-index` explicite passe AU-DESSUS du contenu statique**, pas en dessous. Contrairement aux autres variants (`hero`/`corner`/`scatter`) qui ne sont que quelques icônes dans des zones vides et où `z-index: 0` hérité ne se voit jamais, un calque avec un vrai `background` plein écran DOIT avoir `z-index: -1` explicite (même règle que `.ambient-blobs`), sinon il recouvre le formulaire.
2. **SVG avec un `viewBox` trop juste par rapport aux coordonnées du `path`** (ex : palmier avec des points à `y=-8`, cocktail avec `y=-10`, hors du `viewBox="0 0 W H"` d'origine) rendent correctement en petit (bord de viewBox jamais atteint visuellement) mais se retrouvent nettement rognés une fois l'icône agrandie en gros motif de fond. Fix : élargir le `viewBox` pour couvrir toutes les coordonnées réelles du tracé (`viewBox="-14 -14 148 148"` etc.), pas de `overflow: visible` sur le `<svg>` (moins fiable cross-browser).
3. **Rayon d'arc SVG ≈ 2× la corde → arc quasi plat.** Le motif pastèque initial (`A92 92 0 0 1 92 0`, corde de 92 pour un rayon de 92 au lieu de 46) ne dessinait qu'un fin croissant au lieu d'un demi-cercle plein, illisible une fois agrandi. Pour un vrai demi-cercle plein, le rayon doit être égal à la **moitié** de la largeur de corde (`A48 48 0 0 1` pour une corde de 96).
4. **Éditer `morel-games-core-master/src/components/AskPseudonym.vue`** (bouton icône-only → texte visible `{{ $t(labelButton) }}` + icône, pour un CTA plus lisible) **sans effet immédiat** : même piège que documenté dans INDEX.md, Vite pré-bundle le package dans `node_modules/.vite/deps/` au premier démarrage, un edit après coup ne se recharge pas via HMR. Fix : tuer le process Vite, supprimer `pitit-bac/front/node_modules/.vite/`, relancer.
5. **Le glassmorphism (flou + bordure blanche + coins arrondis) a été jugé "vieillot" par l'utilisateur**, pas juste mal exécuté — deux itérations différentes (carte peinte opaque, puis vrai `backdrop-filter` avec bordure blanche) ont toutes les deux été rejetées comme datées. Direction finale validée par question explicite : **aucune carte/cadre du tout**, le champ et le bouton flottent directement sur le dégradé (fond blanc uni sur l'input, ombre douce, pas de bordure, pas de flou).

**À retenir :** pour ce projet, glassmorphism (flou + bordure translucide) lit comme daté aux yeux de l'utilisateur, quelle que soit l'exécution — préférer soit du flat/minimal sans cadre, soit un cadre à bordure franche et ombre nette (pas de flou), à valider par question explicite plutôt que de deviner entre plusieurs directions "modernes" possibles (elles ne veulent pas dire la même chose pour tout le monde).
